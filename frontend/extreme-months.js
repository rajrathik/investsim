(function () {
  ...
  function render() {
    ...
    $('summaryGrid').innerHTML = `
      <div class="summary-card worst">
        <div class="sc-label">▼ Worst single month</div>
      ...
      <div class="summary-card best">
        <div class="sc-label">▲ Best single month</div>
    ...
    // Panel subtitles
    $('worstSub').textContent = `Ranked by single-month loss · showing top ${_n}`;
    $('bestSub').textContent  = `Ranked by single-month gain · showing top ${_n}`;

    // Tables — bars scale to the #1 entry in each list
  ...
  async function init() {
    ...
    try {
      const res = await fetch('/api/sp500-extreme-months?n=20');
